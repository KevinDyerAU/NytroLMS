...
<head>
    ...
    @auth
            <script>
                ...
                window.Laravel.idleTimeout = {
                    timeoutMinutes: {{ config('idle-timeout.timeout_minutes', 120) }},
                    warningMinutes: {{ config('idle-timeout.warning_minutes', 5) }},
                    checkInterval: {{ config('idle-timeout.check_interval_ms', 60000) }},
                    updateInterval: {{ config('idle-timeout.update_interval_ms', 300000) }}
                };
            </script>
    @endauth
...
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-5Z4WGF9GRD"></script>
<script>
