    ...
    <ul class="nav navbar-nav align-items-center ms-auto">
        @if (!empty($settings['featured_images']['student']['image']))
            <li class="nav-item mx-2">
                <a href="javascript:void(0);" class="fw-bold text-primary" data-bs-toggle="modal"
                    data-bs-target="#studyClubModal"
                    title="{{ $settings['featured_images']['student']['title'] ?? config('settings.site.institute_name', 'Key Institute') . ' - Study Club' }}">
                    <i class="mx-25" data-lucide="book-open"></i>
        ...
        <li class="nav-item dropdown dropdown-language">
            @if (env('SETTINGS_KEY') === 'KeyInstitute')
                @include('frontend.panels.gtranslate')
            @endif
        </li>
            ...
            <li class="nav-item dropdown dropdown-user">
                <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);"
                    ...
                    <div class="user-nav d-sm-flex d-none">
                        <span class="user-name fw-bolder notranslate">{{ auth()->user()->name }}</span>
                    ...
                    <span class="avatar bg-light-primary">
                        ...
                        @else
                            <span
                                class="avatar-content notranslate text-uppercase">{{ ucwords(auth()->user()->first_name[0] . auth()->user()->last_name[0]) }}</span>
                        @endif
