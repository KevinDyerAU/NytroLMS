    ...
    <div class="card-header flex-row align-items-start pb-0">
        <div class='col-2'>
            <div class="avatar bg-success p-50 m-0">
                <a href="{{ route('reports.admins.index', ['course_status' => 'ON SCHEDULE']) }}" class="avatar-content">
                    <i data-lucide="user-check" class="font-medium-5"></i>
                </a>
            </div>
        </div>
        <div class='col-8 d-flex flex-row align-items-center align-self-center'>
            <h3 class="card-text flex-wrap pe-1">All Engaged Students</h3>
            <h2 class="fw-bolder ms-1 ">{{ $data['count'] }}</h2>
        </div>
    </div>
    <div id="engaged-students-chart"></div>
</div>
...
<script>
    (function(window, document, $) {
        $(window).on('load', function() {
            ...
            let $chart = document.querySelector('#engaged-students-chart');
            let chartOptions = {
                chart: {
                    height: 100,
                    width: '100%',
