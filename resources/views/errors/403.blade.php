    ...
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="text-center mt-8">
                @php $logoutUrl = url('logout'); @endphp
                @if(isset($actionUrl) && $actionUrl === $logoutUrl)
                    <form method="POST" action="{{ $actionUrl }}" style="display:inline;">
                        @csrf
                        <button type="submit" class="text-blue-600 underline text-lg" style="background:none;border:none;padding:0;cursor:pointer;">&larr; Logout</button>
                    </form>
                @else
                    <a href="{{ isset($actionUrl) ? $actionUrl : url('/') }}" class="text-blue-600 underline text-lg">&larr; {{ isset($actionTitle) ? $actionTitle : 'Back' }}</a>
                @endif
            </div>
        </div>
    </div>
